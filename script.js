...
const addTaskBtn = $('#addTaskBtn'), dialog = $('#taskDialog'), form = $('#taskForm');
...
function confirmBox({ title='Confirm', message='Are you sure?', okText='OK', cancelText='Cancel' } = {}){
  ...
  dlg.querySelector('#confirmMsg').textContent = message;
  dlg.querySelector('.btn.ok').textContent = okText;
  dlg.querySelector('.btn.cancel').textContent = cancelText;
  return new Promise(resolve=>{
    dlg.addEventListener('close', ()=> resolve(dlg.returnValue === 'ok'), { once:true });
    dlg.showModal(); // centers
  });
}
...
function taskCard(t){
  ...
  btnDel.addEventListener('click', async ()=>{
    const ok = await confirmBox({
      title:'Delete task',
      message:`Move “${t.title}” to Deleted? You can restore it later.`,
      okText:'Delete'
    });
    if(!ok) return;
...
function deletedCard(t){
  ...
  btnPurge.addEventListener('click', async ()=>{
    const ok = await confirmBox({
      ...
      message:`Permanently remove “${t.title}”? This action cannot be undone.`,
...
form.addEventListener('submit', async (e)=>{
  ...
  const payload = {
    userId: authUser.uid, title: titleInput.value.trim(),
    description: (descInput.value || "").trim() || null, priority: priorityInput.value,
    createdAt: base ? base.createdAt : Date.now(), dueAt: dueAt ? dueAt.getTime() : null,
    dueText: (dueTextInput.value || "").trim() || null, status: base ? base.status : 'not_started',
